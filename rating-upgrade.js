...
async function openRating(button) {
  ...
  if (!chapterId) return;
  mounted = true;
  const old = document.querySelector('[data-ar-rating-upgrade]'); old?.remove();
  ...
  try {
    const [rows,my] = await Promise.all([
      supabase.from('chapter_ratings').select('rating').eq('chapter_id',chapterId),
      getMyRating(chapterId).catch(e=>e?.code==='AUTH_REQUIRED'?null:Promise.reject(e)),
    ]);
...
function intercept(event){
  ...
  if(label.startsWith('rate chapter')||label==='rate chapter') { event.preventDefault();event.stopPropagation();event.stopImmediatePropagation?.();openRating(button); }
}
