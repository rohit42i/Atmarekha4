...
async function attachChapters(rows) {
  const ids = [...new Set((rows || []).map(row => row.chapter_id).filter(Boolean))];
  if (!ids.length) return rows || [];
  const { data, error } = await supabase
    .from('chapters')
    .select('id,chapter_number,title,cover_url,status')
    .in('id', ids);
  if (error) throw error;
  const byId = new Map((data || []).map(chapter => [chapter.id, chapter]));
  return (rows || []).map(row => ({ ...row, chapters: byId.get(row.chapter_id) || null }));
}
...
export default function ProfileV2() {
  ...
  const loadActivity = async currentUser => {
    if (!currentUser) return;
    setActivityLoading(true);
    setActivityError('');
    try {
      const [h, b, r] = await Promise.all([
        supabase.from('reading_history').select('id,chapter_id,page_number,updated_at').eq('user_id', currentUser.id).order('updated_at', { ascending: false }).limit(50),
        supabase.from('bookmarks').select('id,chapter_id,created_at').eq('user_id', currentUser.id).order('created_at', { ascending: false }).limit(50),
        supabase.from('chapter_ratings').select('id,chapter_id,rating,created_at').eq('user_id', currentUser.id).order('created_at', { ascending: false }).limit(50)
      ]);
      if (h.error) throw h.error;
      if (b.error) throw b.error;
      if (r.error) throw r.error;
      const [historyRows, bookmarkRows, ratingRows] = await Promise.all([
        attachChapters(h.data),
        attachChapters(b.data),
        attachChapters(r.data)
      ]);
      ...
      setActivityError(err?.message || 'Unable to load your activity.');
      setHistory([]);
      setBookmarks([]);
      setRatings([]);
    } finally {
  ...
  const load = async currentUser => {
    ...
    await loadActivity(currentUser);
  };
  ...
  useEffect(() => {
    if (route === 'profile' && user) loadActivity(user);
  }, [route, user?.id]);
