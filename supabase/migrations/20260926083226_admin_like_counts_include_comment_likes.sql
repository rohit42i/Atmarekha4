...
declare
  ...
  v_result jsonb;
begin
  if v_uid is null or not exists (select 1 from public.admins where user_id = v_uid) then raise exception 'Admin access required'; end if;
  if p_days is not null and p_days not in (1,7,30,90) then raise exception 'Analytics period must be 1, 7, 30, 90, or null'; end if;
