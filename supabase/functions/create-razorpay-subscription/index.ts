...
function getPublishableKey() {
  ...
  try {
    ...
    return typeof parsed === 'string' ? parsed : parsed?.default || Object.values(parsed || {})[0] || '';
  } catch { return ''; }
}
...
async function findOrCreatePlan(plan: { amount: number; name: string }, keyId: string, keySecret: string) {
  ...
  const existing = plans?.items?.find((item: any) =>
    item?.period === 'monthly' &&
    item?.interval === 1 &&
    Number(item?.item?.amount) === plan.amount &&
...
Deno.serve(async req => {
  ...
  try {
    ...
    if (!supabaseUrl || !publishable) return json({ error: 'Supabase public configuration is missing.' }, 500, origin);
    ...
    if (authError || !user) {
      return json({ error: `Authentication failed: ${authError?.message || 'No user found.'}` }, 401, origin);
    }
