...
Deno.serve(async req => {
  ...
  try {
    ...
    if (!paymentId || !orderId || !receivedSignature) {
      return json({ error: 'Missing Razorpay payment verification fields.' }, 400, origin);
    }
    ...
    if (!expectedBytes || !receivedBytes || !timingSafeEqual(expectedBytes, receivedBytes)) {
      return json({ error: 'Payment signature mismatch.' }, 400, origin);
    }
