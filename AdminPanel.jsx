...
export default function AdminPanel({ onLogout }) {
  ...
  async function deleteAnnouncement(item) {
    ...
    setBusy(true);
    try { await requireAdmin(); const { error } = await supabase.from('announcements').delete().eq('title', item.title); if (error) throw error; await load(); }
    catch (error) { setNotice({ type: 'error', text: error.message }); } finally { setBusy(false); }
  }
  ...
  async function deleteMedia(id) {
    ...
    setBusy(true);
    try { await requireAdmin(); const { error } = await supabase.from('media').delete().eq('id', id); if (error) throw error; await load(); }
    catch (error) { setNotice({ type: 'error', text: error.message }); } finally { setBusy(false); }
  }
  ...
  return <main className="admin-dashboard-shell min-h-screen text-[var(--text-color)]"><div className="mx-auto max-w-7xl px-4 py-5 sm:px-6 sm:py-7">
