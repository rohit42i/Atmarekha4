...
export default function AdminPanel({ onLogout }) {
  ...
  async function saveAnnouncement(event) {
    ...
    try { await requireAdmin(); if (!announcement.title.trim() || !announcement.content.trim()) throw new Error('Title and content are required.'); const { error } = await supabase.from('announcements').insert({ title: announcement.title.trim(), content: announcement.content.trim(), image_url: announcement.image_url.trim() || null, is_pinned: announcement.is_pinned, published_at: new Date().toISOString() }); if (error) throw error; setAnnouncement({ title: '', content: '', image_url: '', is_pinned: false }); await load(); setNotice({ type: 'success', text: 'Announcement published.' }); }
    catch (error) { setNotice({ type: 'error', text: error.message }); } finally { setBusy(false); }
  }
  ...
  return <main className="min-h-screen bg-zinc-950 text-white"><div className="mx-auto max-w-7xl px-4 py-5 sm:px-6 sm:py-7">
    ...
    {loading ? <div className="admin-loading">Loading dashboard…</div> : tab === 'Overview' ? <section className="admin-stack">
      <div className="admin-grid four"><Stat label="Total chapters" value={sorted.length}/><Stat label="Published" value={published.length} accent/><Stat label="Manga pages" value={totalPages}/><Stat label="Total views" value={views.length}/></div>
      <div className="admin-grid four"><Stat label="Likes" value={likes.length}/><Stat label="Ratings" value={ratings.length}/><Stat label="Average rating" value={ratings.length ? `${averageRating.toFixed(1)}/10` : '—'}/><Stat label="Comments" value={comments.length}/></div>
      <div className="admin-grid two">
        <section className="admin-card"><div className="admin-card-title"><div><span>RECENT COMMENTS</span><h2>Community</h2></div><button onClick={() => setTab('Comments')}>View all →</button></div><div className="admin-mini-list">{comments.slice(0, 5).map(comment => <div key={comment.id}><strong>{comment.author_name || 'Reader'}</strong><p>{comment.content}</p><small>{chapterName(comment.chapter_id)}</small></div>)}{!comments.length && <p className="muted">No comments yet.</p>}</div></section>
        <section className="admin-card"><div className="admin-card-title"><div><span>REPORTS</span><h2>Moderation queue</h2></div><button onClick={() => setTab('Reports')}>Open queue →</button></div><div className="admin-mini-list">{reports.slice(0, 5).map(report => { const comment = commentById(report.comment_id); return <div key={report.id}><strong>{comment?.author_name || 'Reader'}</strong><p>{comment?.content || 'Comment unavailable'}</p><small>{report.reason || 'Reported by reader'}</small></div>; })}{!reports.length && <p className="muted">No reports. Everything is clean.</p>}</div></section>
      </div>
      <section className="admin-card"><div className="admin-card-title"><div><span>CHAPTER PERFORMANCE</span><h2>Published chapters</h2></div></div><div className="admin-performance">{published.map(chapter => { const chapterRatings = ratings.filter(row => row.chapter_id === chapter.id); const chapterViews = views.filter(row => row.chapter_id === chapter.id).length; const chapterLikes = likes.filter(row => row.chapter_id === chapter.id).length; const avg = chapterRatings.length ? chapterRatings.reduce((sum, row) => sum + Number(row.rating), 0) / chapterRatings.length : 0; return <div key={chapter.id}><div><strong>Chapter {chapter.chapterNumber}</strong><span>{chapter.title}</span></div><p>★ {avg ? avg.toFixed(1) : '—'} · 👁 {chapterViews} · ♥ {chapterLikes} · 📄 {pageCounts[chapter.id] || 0}</p></div>; })}</div></section>
