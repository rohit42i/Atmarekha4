...
async function supabaseRows(env, authorization, table, query) {
  ...
  const response = await fetch(url, {
    headers: { apikey: env.SUPABASE_ANON_KEY, Authorization: authorization, Accept: 'application/json' },
  });
  if (!response.ok) throw new Error(`Supabase authorization check failed (${response.status}).`);
  return response.json();
}
...
function objectKey(request) {
  ...
  let key = '';
  try { key = decodeURIComponent(pathname.slice('/media/'.length)); } catch (_) { return null; }
  ...
  const page = new RegExp(`^chapters/${UUID_PATTERN}/pages/[^/]+/[^/]+const MAX_UPLOAD_BYTES = 20 * 1024 * 1024;
const UUID_PATTERN = '[0-9a-f]{8}-[0-9a-f]{4}-[1-5][0-9a-f]{3}-[89ab][0-9a-f]{3}-[0-9a-f]{12}';
...
function objectKey(request) {
  ...
  const cover = new RegExp(`^covers/chapters/${UUID_PATTERN}/[^/]+$`, 'i');
  , 'i');
  const replacement = new RegExp(`^chapters/${UUID_PATTERN}/replacements/[^/]+const MAX_UPLOAD_BYTES = 20 * 1024 * 1024;
const UUID_PATTERN = '[0-9a-f]{8}-[0-9a-f]{4}-[1-5][0-9a-f]{3}-[89ab][0-9a-f]{3}-[0-9a-f]{12}';
...
function objectKey(request) {
  ...
  const cover = new RegExp(`^covers/chapters/${UUID_PATTERN}/[^/]+$`, 'i');
  , 'i');
  return cover.test(key) || page.test(key) || replacement.test(key) ? key : null;
}
  ...
  async fetch(request, env) {
    ...
    try {
      ...
      if (request.method === 'PUT') {
        ...
        if (length > MAX_UPLOAD_BYTES) return json(request, env, { error: 'Image is larger than 20 MB.' }, 413);
      ...
      return json(request, env, { error: error?.message || 'PDPL media operation failed.' }, 500);
    }
  },
};
