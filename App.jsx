...
function Reader({ chapterId, onBack, chapters }) {
  ...
  useEffect(() => { let cancelled = false; progressHydratedRef.current = false; const load = async () => { setLoading(true); setError(''); try { const all = chapters?.length ? chapters : await buildChapters(); const found = all.find(item => String(item.id) === String(chapterId)); if (!found || !published(found)) throw new Error('Chapter not found or not published.'); const viewPromise = recordChapterView(found.id).catch(viewError => { console.warn('View tracking skipped:', viewError); }); const livePages = await buildChapterPages(found.id); await viewPromise; const engagement = await fetchChapterEngagement(found.id); if (cancelled) return; let restoredIndex = 0; const savedLocal = Number(window.localStorage.getItem(`atma-reading:${found.id}`)); if (Number.isInteger(savedLocal) && savedLocal >= 0 && savedLocal < livePages.length) { restoredIndex = savedLocal; } else { try { const { data: sessionData } = await supabase.auth.getSession(); const userId = sessionData?.session?.user?.id; if (userId) { const { data: history } = await supabase.from('reading_history').select('chapter_id,page_number').eq('user_id', userId).maybeSingle(); if (String(history?.chapter_id) === String(found.id)) { const serverIndex = Number(history?.page_number) - 1; if (Number.isInteger(serverIndex) && serverIndex >= 0 && serverIndex < livePages.length) restoredIndex = serverIndex; } } } catch (historyError) { console.warn('Reading progress restore skipped:', historyError); } } progressHydratedRef.current = true; setChapter(found); setPages(livePages); setStats(engagement); setIndex(restoredIndex); } catch (err) { if (!cancelled) setError(err?.message || 'Unable to load this chapter.'); } finally { if (!cancelled) setLoading(false); } }; load(); return () => { cancelled = true; progressHydratedRef.current = false; }; }, [chapterId, chapters]);
