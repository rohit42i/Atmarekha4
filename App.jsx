  ...
  return (
    <div className="flex min-h-[40vh] items-center justify-center px-6">
      <div className="text-center">
        <div
          className="mx-auto mb-4 h-8 w-8 animate-spin rounded-full border-4 border-zinc-200 border-t-blue-600 dark:border-zinc-700 dark:border-t-blue-400"
          aria-hidden="true"
        />
...
function ChapterReader({ chapterId, onBack }) {
  ...
  return (
    <div className="min-h-screen bg-zinc-50 dark:bg-zinc-950">
      <header className="fixed top-0 z-50 w-full border-b border-zinc-200 bg-white/95 backdrop-blur-md dark:border-zinc-800 dark:bg-zinc-950/95">
        <div className="mx-auto flex max-w-5xl items-center gap-4 px-6 py-4">
          <button
            type="button"
            onClick={onBack}
            aria-label="Back to chapters"
            className="flex h-10 w-10 items-center justify-center rounded-full text-xl text-zinc-500 hover:bg-zinc-100 dark:hover:bg-zinc-800 dark:hover:text-white"
          >
            ←
          </button>
        ...
        {hasPages ? (
          <div className="mx-auto max-w-3xl">
            <img
              ...
              alt={`Chapter ${chapter.chapterNumber}, page ${currentIndex + 1}`}
            ...
            <div className="mt-6 flex items-center justify-between gap-4">
              <button
                ...
                className="flex-1 rounded-xl bg-zinc-200 py-3 font-medium transition hover:bg-zinc-300 disabled:cursor-not-allowed disabled:opacity-30 dark:bg-zinc-800 dark:hover:bg-zinc-700 dark:text-white"
              >
                ← Prev
              </button>
              ...
              <button
                ...
                className="flex-1 rounded-xl bg-zinc-900 py-3 font-medium text-white transition hover:bg-zinc-700 disabled:cursor-not-allowed disabled:opacity-30 dark:bg-white dark:text-black dark:hover:bg-zinc-200"
              >
                Next →
              </button>
            </div>
          ...
          <div className="mx-auto flex min-h-[60vh] max-w-3xl flex-col items-center justify-center rounded-2xl border-2 border-dashed border-zinc-200 bg-white px-6 text-center dark:border-zinc-800 dark:bg-zinc-900">
            ...
            <p className="mt-2 max-w-md text-zinc-500">
              This chapter exists in Supabase, but no manga pages are attached
              to it yet.
            </p>
            <button
              type="button"
...
function ChapterCard({ chapter }) {
  const releaseDate = chapter.releaseDate
    ? new Date(chapter.releaseDate).toLocaleDateString(undefined, {
        day: 'numeric',
        month: 'short',
        year: 'numeric',
      })
  ...
  return (
    <a
      ...
      className="group overflow-hidden rounded-2xl bg-white shadow-sm ring-1 ring-zinc-200 transition hover:-translate-y-1 hover:shadow-xl dark:bg-zinc-900 dark:ring-zinc-800"
    >
      <div className="aspect-[3/4] overflow-hidden bg-zinc-100 dark:bg-zinc-950">
        {chapter.cover ? (
          <img
            src={chapter.cover}
            alt={chapter.title || `Chapter ${chapter.chapterNumber}`}
      ...
      <div className="p-5">
        ...
        {releaseDate && (
          <p className="mt-3 text-xs text-zinc-400">Release: {releaseDate}</p>
        )}
      </div>
    </a>
  );
  ...
  useEffect(() => {
    ...
    async function loadChapters() {
      ...
      try {
        ...
        if (!cancelled) {
          setChapters([]);
          setLoadError('Unable to load chapters from Supabase right now.');
        }
      } finally {
        if (!cancelled) {
          setLoading(false);
    ...
    <main className="min-h-screen bg-zinc-50 text-zinc-900 dark:bg-zinc-950 dark:text-white">
      <header className="border-b border-zinc-200 bg-white px-6 py-5 dark:border-zinc-800 dark:bg-zinc-950">
        <div className="mx-auto flex max-w-7xl items-center justify-between">
          ...
          <nav className="flex gap-5 text-sm text-zinc-600 dark:text-zinc-300">
            <a href="#index" className="hover:text-zinc-900 dark:hover:text-white">
              Home
            </a>
            <a href="#reading" className="hover:text-zinc-900 dark:hover:text-white">
              Chapters
            </a>
          </nav>
        </div>
      ...
      >
        ...
        <p className="mt-2 text-zinc-500 dark:text-zinc-400">
          Chapters are loaded live from Supabase.
        </p>

        {loading && <LoadingState label="Loading chapters..." />}

        {!loading && loadError && (
          <div className="mt-8 rounded-2xl border border-red-200 bg-red-50 p-6 text-red-700 dark:border-red-900/50 dark:bg-red-950/30 dark:text-red-300">
        ...
        {!loading && !loadError && chapters.length === 0 && (
          <div className="mt-8 rounded-2xl border border-dashed border-zinc-300 p-10 text-center dark:border-zinc-700">
            <h3 className="font-semibold">No chapters published yet</h3>
            <p className="mt-2 text-sm text-zinc-500">
              Add a chapter in Supabase and it will appear here automatically.
            </p>
          </div>
        )}
