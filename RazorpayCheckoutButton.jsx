...
export default function RazorpayCheckoutButton({ amount, label = 'Pay with Razorpay', receipt, description = 'Atma Rekha', onSuccess, className = 'membership-button' }) {
  ...
  const startCheckout = async () => {
    ...
    try {
      ...
      const { data, error: invokeError } = await supabase.functions.invoke('create-razorpay-order', { body: { amount: Math.round(rupees * 100), currency: 'INR', receipt }, headers });
      ...
      const checkout = new window.Razorpay({
        ...
        handler: async response => {
          try {
            const { data: verification, error: verificationError } = await supabase.functions.invoke('verify-razorpay-payment', { body: { razorpay_payment_id: response.razorpay_payment_id, razorpay_order_id: response.razorpay_order_id, razorpay_signature: response.razorpay_signature }, headers });
          ...
          } catch (verificationError) { setError(verificationError?.message || 'Payment verification failed.'); }
          finally { setLoading(false); }
        },
      });
      checkout.on('payment.failed', response => { setError(response?.error?.description || 'Razorpay payment failed.'); setLoading(false); });
      checkout.open();
    } catch (checkoutError) { setError(checkoutError?.message || 'Unable to start Razorpay Checkout.'); setLoading(false); }
  };

  return <div><button type="button" className={className} disabled={loading} onClick={startCheckout}>{loading ? 'Opening Razorpay…' : label}</button>{error && <p className="membership-error" role="alert">{error}</p>}</div>;
}
