...
export default function ChapterAccessGuard() {
  ...
  useEffect(() => {
    ...
    const blockLockedChapter = chapter => {
      if (!isLocked(chapter)) return false;
  ...
  useEffect(() => {
    if (!ready) return undefined;
    const markLocks = () => document.querySelectorAll('.chapter-row').forEach(row => {
      const number = Number(row.querySelector('.chapter-row-title span')?.textContent?.match(/(\d+)/)?.[1]);
      if (!Number.isFinite(number) || number <= FREE_CHAPTER_LIMIT) return;
