...
export async function getFavoriteState(chapterId) {
  ...
  const { data, error } = await supabase.from('bookmarks').select('id').eq('user_id', user.id).eq('chapter_id', chapterId).maybeSingle();
  if (error) throw error;
  return Boolean(data);
}
...
export async function toggleFavorite(chapterId) {
  ...
  const existing = await supabase.from('bookmarks').select('id').eq('user_id', user.id).eq('chapter_id', chapterId).maybeSingle();
...
export async function saveReadingProgress(chapterId, pageNumber) {
  ...
  const { error } = await supabase.from('reading_history').upsert({ user_id: user.id, chapter_id: chapterId, page_number: page, updated_at: new Date().toISOString() }, { onConflict: 'user_id,chapter_id' });
  if (error) throw error;
}
