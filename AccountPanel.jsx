...
export default function AccountPanel({ onBack }) {
  ...
  useEffect(() => {
    ...
    const { data: listener } = supabase.auth.onAuthStateChange(async (_event, next) => {
      setSession(next);
      if (next) {
        try { await loadProfile(next.user); } catch (err) { setError(err?.message || 'Unable to load your profile.'); }
      } else {
        setProfile(null);
      }
    });
  ...
  const submit = async event => {
    ...
    try {
      if (mode === 'signin') {
        ...
        const { data, error: authError } = await supabase.auth.signUp({ email: form.email.trim(), password: form.password, options: { data: { username: form.username.trim().slice(0, 40) } } });
        if (authError) throw authError;
        if (data.user) {
          const { error: profileError } = await supabase.from('profiles').upsert({ id: data.user.id, username: form.username.trim().slice(0, 40) || null, updated_at: new Date().toISOString() }, { onConflict: 'id' });
          if (profileError) throw profileError;
        }
        setMessage(data.session ? 'Account created.' : 'Account created. Check your email to verify it, then sign in.');
        setMode('signin');
      }
      setForm(emptyForm);
    } catch (err) {
      setError(err?.message || 'Unable to complete that request.');
  ...
  const signOut = async () => { setSaving(true); await supabase.auth.signOut(); setSaving(false); setMessage('Signed out.'); };
  const deleteAccount = async () => {
    setError('Account deletion is not available from the browser yet.');
  };
