...
export default function AdminGroupChatTools() {
  const [user, setUser] = useState(null), [isAdmin, setIsAdmin] = useState(false), [open, setOpen] = useState(false);
  ...
  const deleteIds = async ids => {
    if (!ids.length || deleting) return; setDeleting(true); setNotice({ type:'', text:'' });
    try { await verifyAdmin(); for (const table of DEPENDENCY_TABLES) { try { await supabase.from(table).delete().in('message_id', ids); } catch (_) {} }
      const { error }=await supabase.from('group_chat_messages').delete().in('id',ids); if(error) throw error;
      setMessages(prev=>prev.filter(m=>!ids.includes(m.id))); setSelected(new Set()); setNotice({type:'success',text:`${ids.length} message${ids.length===1?'':'s'} deleted.`});
    } catch(error){setNotice({type:'error',text:error.message||'Unable to delete message(s).' });} finally{setDeleting(false);}
  };
