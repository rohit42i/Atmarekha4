...
function syncBellVisibility() {
  ...
  // If this browser cannot request notifications, or the reader has blocked them,
  // remove the bell rather than showing a dead control.
...
async function requestChapterNotifications() {
  if (!('Notification' in window)) return syncBellVisibility();
  if (Notification.permission === 'denied') return syncBellVisibility();
  const permission = Notification.permission === 'granted' ? 'granted' : await Notification.requestPermission();
  if (permission !== 'granted') return;
  localStorage.setItem('atma-rekha-notifications-enabled', '1');
  syncBellVisibility();
  try {
    new Notification('Atma Rekha notifications enabled', { body: 'You will be notified about new chapter updates.', icon: '/favicon.png', badge: '/favicon.png', tag: 'atma-rekha-notification-enabled' });
  } catch {}
...
async function establishBaselines() {
  try {
    ...
    if (response.ok) { const payload = await response.json(); const items = Array.isArray(payload?.data) ? payload.data : []; if (items[0]) localStorage.setItem(ANNOUNCEMENT_KEY, newestId(items[0])); }
...
async function checkForUpdates() {
  ...
  try {
    const response = await fetch(`${API_BASE}/api/announcements`, { cache: 'no-store' });
    if (response.ok) { const payload = await response.json(); const items = Array.isArray(payload?.data) ? payload.data : []; const newest = items[0]; if (newest) { const id = newestId(newest); const previous = localStorage.getItem(ANNOUNCEMENT_KEY); if (previous && id && previous !== id) { localStorage.setItem(ANNOUNCEMENT_KEY, id); showNotification(newest.title || 'Atma Rekha update', newest.content || 'A new update is available.', id); } else if (!previous && id) localStorage.setItem(ANNOUNCEMENT_KEY, id); } }
  } catch (error) { console.warn('Announcement check skipped:', error); }
  try {
    const chapters = (await buildChapters()).filter(chapter => String(chapter?.status || '').toLowerCase() === 'published').sort((a, b) => Number(b.chapterNumber) - Number(a.chapterNumber));
    const newest = chapters[0]; if (!newest) return; const id = newestId(newest) || String(newest.chapterNumber); const previous = localStorage.getItem(CHAPTER_KEY);
    if (previous && id !== previous) { localStorage.setItem(CHAPTER_KEY, id); showNotification(`Chapter ${newest.chapterNumber} is live!`, newest.title ? `${newest.title} is now available to read.` : 'A new Atma Rekha chapter is now available.', `chapter-${id}`); }
    else if (!previous) localStorage.setItem(CHAPTER_KEY, id);
  } catch (error) { console.warn('Chapter check skipped:', error); }
}
...
function installNotificationFix() {
  document.addEventListener('click', event => {
    ...
    if (!button) return;
    event.preventDefault(); event.stopImmediatePropagation(); requestChapterNotifications();
  }, true);
  syncBellVisibility();
