...
export default function AdminOverview({ chapters, comments, ratings, views, likes, pageCounts, onTab, chapterName }) {
  ...
  return <section className="admin-overview">
    ...
    <div className="admin-overview-grid">
      ...
      <section className="admin-overview-card top-chapters-card"><div className="admin-overview-card-head"><div><span>TOP CHAPTERS · BY VIEWS</span><h3>Best performing</h3></div><button type="button" onClick={() => onTab('Chapters')}>View all →</button></div><div className="admin-top-chapters">{[...metrics.chapterStats].sort((a,b)=>b.views-a.views).slice(0, 5).map((chapter, index) => { const avg = Number(chapter.ratingAverage || 0); return <button type="button" key={chapter.id} onClick={() => onTab('Chapters')}><b>{index + 1}.</b><div><strong>Chapter {chapter.chapterNumber} — {chapter.title}</strong><span>{formatNumber(chapter.views)} views · ♥ {formatNumber(chapter.likes)} · ★ {avg ? avg.toFixed(1) : '—'}</span></div><i>›</i></button>; })}{!metrics.chapterStats.length && <p className="admin-overview-empty">No published chapters yet.</p>}</div></section>
    </div>
    <section className="admin-overview-card admin-chapter-views-card">
      ...
      <div className="admin-chapter-views-table"><div className="admin-chapter-views-row admin-chapter-views-head"><span>Chapter</span><span>Total views</span><span>{periodLabel}</span></div>{[...metrics.chapterStats].sort((a,b)=>(Number(a.chapterNumber||0)-Number(b.chapterNumber||0))).map(chapter=><div className="admin-chapter-views-row" key={chapter.id}><span><strong>{chapter.chapterNumber == null ? 'Unnumbered' : 'Chapter ' + chapter.chapterNumber}</strong><small>{chapter.title || 'Untitled'}</small></span><b>{formatNumber(chapter.views)}</b><b>{formatNumber(chapter.periodViews)}</b></div>)}{!metrics.chapterStats.length&&<p className="admin-overview-empty">No published chapters yet.</p>}</div>
    </section>
