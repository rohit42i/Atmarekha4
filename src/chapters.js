...
export async function buildChapterPages(chapterId) {
  ...
  // Supabase can occasionally return a transient empty/error response. Do not
  // immediately treat that as a chapter with no pages; retry automatically.
  for (let attempt = 1; attempt <= PAGE_FETCH_ATTEMPTS; attempt += 1) {
    ...
    if (!error) {
      ...
      // Return immediately when pages exist. If the response is empty, give
      // Supabase a few more chances before reporting an actually empty chapter.
      if (pages.length > 0 || attempt === PAGE_FETCH_ATTEMPTS) {
        return pages;
      }
    } else {
      lastError = error;
      console.warn(
        `Supabase chapter pages attempt ${attempt}/${PAGE_FETCH_ATTEMPTS} failed:`,
        error
      );
