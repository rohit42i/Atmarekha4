...
export default function Membership() {
  ...
  const loadSubscription = async currentUser => {
    ...
    const { data } = await supabase.from('user_subscriptions')
      .select('plan_id,status,current_period_start,current_period_end,cancel_at_period_end,provider_subscription_id')
  ...
  const beginCheckout = async plan => {
    ...
    const normalizedPhone = rawPhone.replace(/^\+91/, '').slice(-10);
    ...
    } catch (err) { setError(err?.message || 'Unable to start membership checkout.'); }
  ...
  const cancelMembership = async () => {
    ...
    try {
      const { data, error: invokeError } = await supabase.functions.invoke('cancel-cashfree-subscription-v2', { body: { subscription_id: subscription.provider_subscription_id } });
      if (invokeError) throw invokeError;
      if (!data?.ok) throw new Error(data?.error || 'Unable to cancel the subscription.');
      setMessage('Cancellation scheduled. No further renewal will be taken.');
    ...
    <div className="membership-overlay" role="dialog" aria-modal="true" aria-label="Atma Rekha membership">
      ...
      <section className="membership-sheet">
        <header className="membership-head">
          ...
          <div><p className="membership-kicker">ATMA REKHA · MEMBERSHIP</p><h1>Choose your place in the journey.</h1><p>{freePeriod ? `Membership is completely optional until ${formatDate(graceEnd)}.` : 'Chapters 1–7 remain free forever. Membership unlocks Chapter 8 onward.'}</p></div>
        </header>
        ...
        <div className="membership-grid">{PLANS.map(plan => <PlanCard key={plan.id} plan={plan} current={plan.id === 'free' ? !active : active && subscription.plan_id === plan.id} busy={loading && selected?.id === plan.id} onChoose={() => plan.id === 'free' ? (window.location.hash = 'home') : choosePaidPlan(plan)} />)}</div>
        ...
        {needsPhone && <div className="membership-phone"><div><strong>Mobile number for UPI AutoPay</strong><span>Used only to create your Cashfree subscription mandate.</span></div><input value={phone} onChange={event => setPhone(event.target.value.replace(/\D/g, '').slice(0, 10))} inputMode="numeric" autoComplete="tel" placeholder="10-digit mobile number" aria-label="Mobile number" /><button type="button" disabled={phone.length !== 10 || loading} onClick={() => beginCheckout(selected)}>Subscribe securely</button></div>}
...
function PlanCard({ plan, current, busy, onChoose }) {
  return <article className={`membership-plan ${plan.popular ? 'is-popular' : ''} ${current ? 'is-current' : ''}`}>
    ...
    <div className="membership-plan-icon">{plan.icon}</div><h2>{plan.name}</h2>
    <p className="membership-price">₹{plan.amount}<small>{plan.amount ? ' / month' : ''}</small></p>
    <p className="membership-description">{plan.description}</p><ul>{plan.features.map(feature => <li key={feature}>✓ {feature}</li>)}</ul>
    <button type="button" className={`membership-button ${current ? 'current' : ''}`} disabled={current || busy} onClick={onChoose}>{current ? 'Current plan' : busy ? 'Opening checkout…' : plan.amount ? `Subscribe · ₹${plan.amount}/month` : 'Read Chapters 1–7'}</button>
  </article>;
}
...
function MembershipLauncher({ user }) {
  ...
  useEffect(() => {
    ...
    const mount = () => {
      ...
      if (!slot) { slot = document.createElement('div'); slot.className = 'profile-membership-slot'; community.insertAdjacentElement('afterend', slot); }
