  ...
  const load = async () => {
    ...
    try {
      const [{ data: reportRows, error: reportError }, { data: userRows, error: userError }] = await Promise.all([
        supabase.from('moderation_reports').select('id,user_id,source_type,source_id,reason,offense_count,status,reviewed_at,reviewed_by,created_at').order('created_at', { ascending: false }).limit(100),
    ...
    {open && <div className="ar-mod-backdrop" onMouseDown={e => e.target === e.currentTarget && setOpen(false)}>
      <section className="ar-mod-modal" role="dialog" aria-modal="true" aria-label="Community moderation">
        ...
        <div className="ar-mod-body">
          <section className="ar-mod-section"><h3>Automatic reports · {reports.filter(r => r.status === 'open').length} open</h3>{reports.length ? reports.map(report => { const p=profiles[report.user_id]; const name=p?.display_name||p?.username||'Reader'; return <article className="ar-mod-card" key={report.id}><div className="ar-mod-row"><div className="ar-mod-avatar">{p?.avatar_url?<img src={p.avatar_url} alt=""/>:name.slice(0,1).toUpperCase()}</div><div className="ar-mod-main"><div className="ar-mod-name">{name}</div><div className="ar-mod-meta">{report.source_type} · {report.offense_count} total flagged uses · {new Date(report.created_at).toLocaleString('en-IN')}</div></div>{report.status==='open'&&<button className="ar-mod-btn ok" type="button" disabled={busy===`report:${report.id}`} onClick={()=>updateReportStatus(report,'reviewed')}>{busy===`report:${report.id}`?'…':'Mark reviewed'}</button>}{report.status!=='resolved'&&<button className="ar-mod-btn ok" type="button" disabled={busy===`report:${report.id}`} onClick={()=>updateReportStatus(report,'resolved')}>Resolve</button>}{report.status!=='open'&&<button className="ar-mod-btn" type="button" disabled={busy===`report:${report.id}`} onClick={()=>updateReportStatus(report,'open')}>Reopen</button>}}</div><div className="ar-mod-report">{report.reason}<br/><small>Source: {report.source_id}{report.reviewed_at ? ' · reviewed ' + new Date(report.reviewed_at).toLocaleString('en-IN') : ''}</small></div></article>}) : <div className="ar-mod-empty">No automatic reports.</div>}</section>
